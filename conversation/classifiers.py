...
Categories:
  ...
  help_abuse         — Trying to short-circuit the Socratic process:
                       "just tell me", "what's the answer", "idk" with
                       no attempt, "skip", "make it easier".
...
Output STRICT JSON only — no markdown, no preamble:
{
  "verdict": "on_topic_engaged" | "help_abuse" | "off_domain" |
             "deflection" | "opt_in_yes" | "opt_in_no" |
             "opt_in_ambiguous",
...
def haiku_intent_classify_unified(
    ...
    *,
    ...
    if (
        verdict not in {"on_topic_engaged", "opt_in_yes", "opt_in_no", "opt_in_ambiguous"}
        and evidence

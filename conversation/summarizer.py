"""
Summarizes the early turns of a long session into a single
system-role paragraph so the rendered history stays bounded.

Triggered from `dean_node` once
`turn_count >= max_turns - summarizer_keep_recent`. The most recent
`summarizer_keep_recent` turns (default 10) are always preserved
verbatim; everything before them is replaced with a single paragraph
generated by `cfg.models.summarizer`. The summary deliberately omits
the locked answer.

Replacing the early messages invalidates the cached history block
for that turn. This is acceptable near the session cap because the
post-summary history becomes a new stable prefix that caches again
on subsequent turns. Don't move summarization earlier in the session
without re-examining the cache trade-off.
"""
...
"""
    ...
    # : split the prompt into a stable instruction prefix +

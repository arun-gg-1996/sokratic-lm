"""
Endpoints for reading and clearing one student's cross-session memory.

  GET    /api/memory/{student_id}   — return what mem0 has on the student
  DELETE /api/memory/{student_id}   — wipe the student's mem0 namespace

Operates on a single student only; the global clear path is kept off
the public API so one user can't drop another user's memories.
"""

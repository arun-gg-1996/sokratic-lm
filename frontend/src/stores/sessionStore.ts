...
interface SessionState {
  ...
  closeReason: string;
  // N8 (POST_DEMO_FIXES.md, 2026-05-06): "Suggest answers" toggle for
  // QA / demo testing. When enabled, each tutor reply spawns an
  // SuggestionBubbles row with 4 intent-class-diverse student replies
  // generated by Haiku in the chosen profile's voice. Persisted to
  // localStorage so the toggle survives reloads during a demo.
  suggestEnabled: boolean;
  suggestProfile: string;
  setSuggestEnabled: (v: boolean) => void;
  setSuggestProfile: (p: string) => void;
...
export const useSessionStore = create<SessionState>((set) => ({
  ...
  closeReason: "",
  // N8 — initialize from localStorage so the toggle persists across
  // page reloads. Defaults to off / S2 (Moderate profile).
  suggestEnabled:
    typeof window !== "undefined" &&
    window.localStorage?.getItem("sokratic.suggestEnabled") === "1",
  suggestProfile:
    (typeof window !== "undefined" &&
      window.localStorage?.getItem("sokratic.suggestProfile")) ||
    "S2",
  setSuggestEnabled: (v) => {
    try {
      window.localStorage?.setItem("sokratic.suggestEnabled", v ? "1" : "0");
    } catch { /* no-op */ }
    set({ suggestEnabled: v });
  },
  setSuggestProfile: (p) => {
    try {
      window.localStorage?.setItem("sokratic.suggestProfile", p);
    } catch { /* no-op */ }
    set({ suggestProfile: p });
  },
